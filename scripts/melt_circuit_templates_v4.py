...
Uses manual CSV parsing to extract JSON properly.
"""
...
def parse_csv_line_preserving_json(line):
    ...
    Strategy:
    1. Split by comma only when outside quotes
    2. Detect JSON arrays by looking for [ and ]
    3. Don't split JSON arrays
    """
    fields = []
    current_field = []
    in_quotes = False
    ...
    while i < len(line):
        ...
        # Split on comma only when:
        # - Not in quotes
        # - Not in JSON array (or at depth 0)
