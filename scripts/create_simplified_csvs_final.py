...
def simplify_movements():
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        ...
        for row_num, row in enumerate(reader, 1):
            # Remove embedding_vector and None fields
            simplified_row = {}
            for k, v in row.items():
                if k not in ['embedding_vector', 'None']:
                    # Handle different value types
                    if isinstance(v, str):
                        simplified_row[k] = v if v.strip() else ''
                    elif v:
                        simplified_row[k] = v
                    else:
                        simplified_row[k] = ''
    ...
    # Write simplified CSV - exclude embedding_vector and None from headers
    new_headers = [h for h in reader.fieldnames if h not in ['embedding_vector', 'None']]
