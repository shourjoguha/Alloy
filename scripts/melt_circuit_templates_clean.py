...
Uses manual CSV parsing to extract JSON properly and remove all complex JSON columns.
"""
...
def parse_csv_line_preserving_json(line):
    """
    Parse a CSV line manually, preserving JSON arrays and objects.
    """
    fields = []
    current_field = []
    in_quotes = False
    ...
    while i < len(line):
        ...
        # Split on comma only when not in quotes and not in JSON
        if char == ',' and not in_quotes and json_bracket_depth == 0 and json_curly_depth == 0:
...
def melt_circuit_templates():
    ...
    output_file = Path(__file__).parent.parent / "Manual-CSV Upload" / "circuit_templates_melted_clean.csv"
    ...
    # Columns to exclude (all complex JSON)
    exclude_columns = ['exercises_json', 'tags', 'bucket_stress', 'muscle_volume', 'muscle_fatigue',
                     'fatigue_factor', 'stimulus_factor', 'total_reps', 'estimated_work_seconds', 'effective_work_volume']
    ...
    # Define headers for melted CSV (excluded all complex JSON columns)
    headers = [
    ...
    print("❌ Removed ALL complex JSON columns (exercises_json, tags, bucket_stress, muscle_volume, muscle_fatigue, fatigue_factor, stimulus_factor, total_reps, estimated_work_seconds, effective_work_volume)")
    print()
...
def create_melted_row(circuit_row, exercise, seq_num, total_exercises):
    ...
    row = {
        ...
        # Additional circuit fields (non-JSON only)
...
def main():
    ...
    print()
    print("📋 Next Steps:")
    print("1. Import circuit_templates_melted_clean.csv into Supabase")
