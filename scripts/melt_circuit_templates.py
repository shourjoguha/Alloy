...
- One row per exercise in the circuit
...
def melt_circuit_templates():
        ...
        for row_num, row in enumerate(reader, 1):
            ...
            if exercises_json and exercises_json.strip():
                try:
                    ...
                    print(f"⚠️  Row {row_num}: Failed to parse exercises_json for circuit {circuit_id}")
                    print(f"   Error: {e}")
    ...
    print(f"✅ Melted {len(melted_rows)} rows from {row_num} circuits")
    print(f"📁 Output: {output_file}")
    print()
    print("📊 Statistics:")
    print(f"   - Original circuits: {row_num}")
    print(f"   - Melted rows: {len(melted_rows)}")
    print(f"   - Avg exercises per circuit: {len(melted_rows) / row_num:.1f}")
    print()
